// Hand-written "golden" SystemVerilog model of the 5stage_gen_cache_cpu_ref
// datapath, for combinational equivalence checking against the Ivy-generated
// RTL (5stage_gen_cache_cpu_ref.il). Register and memory names match the Ivy
// model exactly (after flatten) so yosys equiv_make can pair them up.
//
// This is the module-based cache CPU: the memory subsystem is the reusable
// `idcache` module, instantiated as `idc`, whose internals (main memory + a
// two-word-line I-cache + a write-back D-cache) are the sub-instances main_mem,
// ic, dc. The proof-only interface isolates (mem_ic/mem_dc and their props) hold
// no RTL state, so they are omitted here. The gen CPU is single-issue, so idc
// exports only fetch_data0/fetch_valid0 (lane-1 fetch outputs are unused and
// omitted). rst is tied to 0 for the check (the Ivy reset is a per-register mux).
//
// Bit layouts:
...
// concat(a,b,..) is {a,b,..} (first arg = MSBs).

// ===========================================================================
// CPU top: pipeline datapath + register file, instantiating bp and idc.
// ===========================================================================
module cpu ( \posedge , rst );
    ...
    //      present so the golden model can also be simulated) ----
    integer ri;
    initial begin
        pc = 8'd0;
    ...
    idcache idc (
        ...
        .fetch_addr(pc),
        .read_req  (m_valid & (m_opcode==3'd4)),
        .write_req (m_valid & (m_opcode==3'd5)),
        .flush_req (m_valid & (m_opcode==3'd7)),
        .data_addr (m_addr),
        .write_data(m_store),
        .fetch_data0 (idc_fetch_data0),
        .fetch_valid0(idc_fetch_valid0),
        .read_data   (idc_read_data),
        .data_stall  (idc_data_stall)
    );
    ...
    wire [15:0] w_val_next = (m_opcode != 3'd4) ? m_res : idc_read_data;
...
// ===========================================================================
// idcache: wires up main_mem + ic + dc into the read/write/flush + fetch ports.
// ===========================================================================
module idcache (
    \posedge , rst,
    fetch_req, fetch_addr, read_req, write_req, flush_req, data_addr, write_data,
    fetch_data0, fetch_valid0, read_data, data_stall
);
...
module ic (
    ...
    always @(posedge \posedge ) begin
        // Priority to FLUSH: any flush cycle suppresses the fill install (the fill
        // holds and re-reads next cycle), so the icache fill-install and flush-
        // eviction writers are mutually exclusive -- at most one icache write/cycle.
