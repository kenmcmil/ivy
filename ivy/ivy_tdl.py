...
def herbrandize_property_vars(prop_z3_fm):
    if prop_z3_fm.is_forall():
        ...
        for i in range(prop_z3_fm.num_vars()):
            ...
            tdl_str = f"(declare-fun {var_name} () {var_sort})"
...
def check_isolate(method="mc",tagged_dfns=[],use_array_encoding=True):
    ...
    for (name,init) in mod.initializers:
        if has_assert(init):
            raise iu.IvyError(x,'ivy_tdl cannot handle assertions in initializers')
...
#    action = ia.EnvAction(*ext_acts)
            ...
            if il.is_constant(sym):
                if is_func_decl(decl):
                    ...
                    if not il.is_interpreted_symbol(sym):
                        sign = "(-> "
                        for i in range(decl.arity()):
                            sign += ' ' + decl.domain(i).sexpr()
                        sign += decl.range().sexpr() + ')'
                        vmt_var_defs['|'+str(decl)+'|'] = f'(fun |{str(decl)}| {sign})'
                else:
                    add_sort(decl.sort())
                    if not il.is_interpreted_symbol(sym):
                        decl_sexpr = decl.sexpr()
                        decl_sort = decl.sort().sexpr()
                        vmt_var_defs[decl_sexpr] = f'(var {decl_sexpr} {str(decl_sort)})'
            else:
                vmt_var_defs[decl.sexpr()] = ('(vmtlib ' + str(decl.sexpr()) + ')')
    vmt_sort_defs = []
    vmt_func_defs = []
    ...
    for sort in sorts:
        ...
        if sort_name.startswith("Arr"):
            ...
            read = f"(vmtlib (declare-fun Read{sort_name} ({sort_name} {dom}) {rng}))"
            write = f"(vmtlib (declare-fun Write{sort_name} ({sort_name} {dom} {rng}) {sort_name}))"
            const = f"(vmtlib (declare-fun Const{sort_name} ({rng}) {sort_name}))"
            vmt_func_defs.append(read)
            vmt_func_defs.append(write)
            vmt_func_defs.append(const)
        if isinstance(sort,DatatypeSortRef):
            sort_str = f"(vmtlib (declare-datatypes () (({sort_name} {' '.join(str(sort.constructor(i)) for i in range(sort.num_constructors()))}))))"
        else: 
            if method=='fsmc': ### TODO: how to pass in the size of the finite-state sort???
                sort_str = f"(finite-sort {sort_name} 5)"
            else:
                sort_str = f"(sort {sort_name})"
    ...
    for sym in ilu.used_symbols_asts([init,trans]):
        if tr.is_new(sym): # any var with a corresponding new_-prefixed variableshould be annotated as a state var  
            ...
            if is_func_decl(cur_sym):
                ...
                full_str = f"(state-fun {cur_name} (-> {decl_dom} {decl_sort}) :next ${next_name})"
                # full_str = f"(define-fun .{cur_name} ({decl_dom}) {decl_sort} (! {cur_name} :next {next_name}))"
                vmt_var_defs[cur_name] = full_str
            else: # not a function --- a variable.
                sort = cur_sym.sort().sexpr()
                full_str = f"(state-var {cur_str} {sort} :next {next_str})"
                vmt_var_defs[cur_str] = full_str
                # full_str = f"(define-fun .{cur_str} () {sort} (! {cur_str} :next {next_str}))"
        elif "fml:" in sym.name or sym.is_skolem() or sym in immutable: # transition variable 
            ...
            full_str = f"(transition-var {cur_sym_str} {sort})"
            vmt_var_defs[cur_sym_str] = full_str
            #full_str = f"(define-fun .{define_fun_str} () {sort} (! {cur_sym_str} :next {next_sym_str}))"

    # frame condition for immutable variables
    frame = [il.Equals(x,tr.new(x)) for x in immutable]
    print ('frame: {}'.format(frame))

    # now iterate through var definitions and remove next-state definitions
    # this is specific to TDL, where both curr and next-state names for
    # a state variable is defined in a single (state-var ...) statement
    reduced_vmt_var_defs = {}
    for entry in vmt_var_defs:
        if not(entry.startswith("new_") or entry.startswith("|new_")):
            reduced_vmt_var_defs[entry] = vmt_var_defs[entry]
    vmt_var_defs = reduced_vmt_var_defs # override

    #for entry in vmt_var_defs:
    #    print("VMT DEF ", entry, "\t", vmt_var_defs[entry])
    #exit(0)
    props = []
    ...
    for i, lf in enumerate(conjs):
        ...
        prop_tdl = f'(property {prop_formula} :name invar_{str(i)})'
        props.append(prop_tdl)
    ...
    init_vmt = f"(init {init_formula_str})"
    ...
    vmt_axiom_defs = []
    for axiom in axioms:
        vmt_axiom_defs.append('(theory-axiom {}'.format(simplify(slvr.formula_to_z3(axiom)).sexpr()))
        #vmt_axiom_defs.append('(assert {})'.format(simplify(slvr.formula_to_z3(axiom)).sexpr()))

    vmt_dfns = []
    print("tagged_dfns:", tagged_dfns)
    
    for tag, dfn in tagged_dfns: # TODO: support exporting multiple liveness proof goals in the future
        if str(tag) == "react_p":
            rhs = slvr.term_to_z3(dfn.rhs()).sexpr()
            vmt_dfns.append(f"(live-assumption {rhs} :name live0)")
        elif str(tag) == "react_q":
            rhs = slvr.term_to_z3(dfn.rhs()).sexpr()
            vmt_dfns.append(f"(live-property {rhs} :name live0)")
        elif str(tag) == "react_r":
            rhs = slvr.term_to_z3(dfn.rhs()).sexpr()
            vmt_dfns.append(f"(fair-assumption {rhs} :name live0)")
        else: 
            #print(f'dfn: {type(dfn)}')
            args = " ".join(f"({x}.sexpr() {x.sort().sexpr()})" for y in dfn.lhs().args for x in [slvr.term_to_z3(y)])
            rng = slvr.term_to_z3(dfn.lhs()).sort().sexpr()
            rhs = slvr.term_to_z3(dfn.rhs()).sexpr()
            thing = f"(vmtlib (define-fun {dfn.defines()} ({args}) {rng} (! {rhs} :{tag} true)))" # thing is something we don't know, so we wrap with (vmtlib ...)
            vmt_dfns.append(thing)

    vmt_file_name = mod.name + '.tdl'
    vmt_var_defs_str = []
    for x in vmt_var_defs:
        vmt_var_defs_str.append(vmt_var_defs[x])
    vmt_var_defs_str.sort() 
    with open(vmt_file_name, "w") as f:
        vmt_str = vmt_sort_defs + vmt_func_defs + vmt_var_defs_str + vmt_formula_defs + vmt_axiom_defs + vmt_dfns
        f.write("\n".join(vmt_str))
...
def get_trans_str(trans_fm_list, actions, herbrand_trans):
    ...
    for action, trans_fm in zip(actions, trans_fm_list):
        ...
        if len(herbrand_trans) > 0:
            frame = " ".join(herbrand_trans)
            trans_str = "(and " + trans_str + " " + frame + ")" 
        full_str = f"(transition {trans_str} :name {action_name})"
